...
**用語の整理**: ここでの SSoT は **ドメイン／基盤ロジック** を指す。**Protobuf メッセージ形**のワイヤ契約の SSoT は submodule **`3rdparty/alchemy-protocol/proto`**。UDP や Phoenix JSON など **別形式**は別の SSoT（[.workspace/0_docs/architecture/overview.md](../../.workspace/0_docs/architecture/overview.md#設計思想)）。
