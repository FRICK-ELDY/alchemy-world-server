...
**用語の整理**: ここでの SSoT は **ドメイン／基盤ロジック** を指す。**Protobuf メッセージ形**のワイヤ契約の SSoT は [alchemy-protocol](https://github.com/FRICK-ELDY/alchemy-protocol)（本リポは `PROTOCOL_PIN`）。UDP や Phoenix JSON など **別形式**は別の SSoT。
