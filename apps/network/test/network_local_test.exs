defmodule Network.LocalTest do
  @moduledoc """
  Network.Local の単体テスト。
  ...
  setup do
    ...
    # Network.Application（Network.Local を含む）が既に起動している。
    # Network.Local は名前付きプロセスのため、各テストで独立したインスタンスを
  ...
  describe "ルーム管理" do
    ...
    test "connect_rooms は存在しないルームに対してエラーを返す" do
      assert {:error, {:room_not_found, "ghost"}} =
               Network.Local.connect_rooms("ghost", "also_ghost")
    end

    test "connected? は未接続のルームに false を返す" do
      refute Network.Local.connected?("a", "b")
    end

    test "disconnect_rooms は存在しないルームに対してゴーストエントリを作成しない" do
      :ok = Network.Local.disconnect_rooms("ghost_a", "ghost_b")
      refute "ghost_a" in Network.Local.list_rooms()
      refute "ghost_b" in Network.Local.list_rooms()
    end
  end
  ...
  describe "接続管理（StubRoom を使用）" do
    ...
    test "connect_rooms → broadcast でイベントが届く", %{pid_a: pid_a, pid_b: pid_b} do
      ...
      assert Network.Local.connected?("room_a", "room_b")
      assert Network.Local.connected?("room_b", "room_a")

      :ok = Network.Local.broadcast("room_a", :ping)
    ...
    test "disconnect_rooms で接続が解除される" do
      ...
      :ok = Network.Local.disconnect_rooms("room_a", "room_b")
      refute Network.Local.connected?("room_a", "room_b")
      refute Network.Local.connected?("room_b", "room_a")
    end

    test "broadcast は接続のないルームに対してエラーを返す" do
      assert {:error, :room_not_found} =
               Network.Local.broadcast("nonexistent", :event)
    end
  ...
  describe "イベント受信の同期確認（notify オプション使用）" do
    test "broadcast で接続先には届き、送信元には届かない" do
      ...
      :ok = Network.Local.connect_rooms("notify_a", "notify_b")

      :ok = Network.Local.broadcast("notify_a", :hello)
  ...
  describe "OTP 隔離" do
    test "一方のルームプロセスがクラッシュしても他方は継続する" do
      ...
      :ok = Network.Local.connect_rooms("iso_a", "iso_b")
      ...
      assert :ok = Network.Local.broadcast("iso_b", :still_alive)
    end
  end
  ...
  # Network.Local の接続テーブルにルームを登録し、on_exit でクリーンアップを登録する。
