defmodule GameEngine.GameEvents do
  ...
  def handle_cast({:select_weapon, :__skip__}, state) do
    ...
    case GameEngine.SceneManager.current() do
      {:ok, %{module: ^level_up_scene}} ->
        ...
        update_playing_scene_state(&rule.apply_level_up_skipped/1)
        {:noreply, state}
  ...
  def handle_cast({:select_weapon, weapon}, state) do
    ...
    case GameEngine.SceneManager.current() do
      {:ok, %{module: ^level_up_scene}} ->
        weapon_id = current_world().entity_registry().weapons[weapon] ||
        ...
        GameEngine.NifBridge.resume_physics(state.control_ref)
        GameEngine.SceneManager.pop_scene()
        update_playing_scene_state(&rule.apply_weapon_selected(&1, weapon))
        {:noreply, state}
  ...
  defp handle_ui_action_skip(state) do
    rule = current_rule()
    playing_state = get_playing_scene_state()
  ...
  defp handle_ui_action_weapon(state, weapon_name) do
    ...
    if is_level_up_pending do
      selected_weapon = resolve_weapon_from_name(weapon_name, current_weapon_levels, rule, state.world_ref)
  ...
  defp resolve_weapon_from_name(weapon_name, weapon_levels, rule, world_ref) when is_binary(weapon_name) do
    requested_weapon =
      try do
    ...
    weapons_registry = current_world().entity_registry().weapons
    ...
    cond do
      is_atom(requested_weapon) and MapSet.member?(allowed_weapons, requested_weapon) ->
        ...
        update_playing_scene_state(&rule.apply_weapon_selected(&1, requested_weapon))
        requested_weapon
      ...
      MapSet.member?(allowed_weapons, fallback_weapon) ->
        ...
        update_playing_scene_state(&rule.apply_weapon_selected(&1, fallback_weapon))
        fallback_weapon
  ...
  defp handle_frame_events_main(events, state) do
    ...
    case GameEngine.SceneManager.current() do
      ...
      {:ok, %{module: mod, state: scene_state}} ->
        ...
        state = process_transition(result, state, now, rule)
  ...
  defp maybe_level_up(state) do
    ...
    if state.exp >= required and required > 0 do
      ...
      unless already_pending do
        ...
        update_playing_scene_state(&rule.apply_level_up(&1, choices))
      end
  ...
  defp process_transition({:continue, _}, state, _now, _rule), do: state
  defp process_transition({:continue, _, _}, state, _now, _rule), do: state

  defp process_transition({:transition, :pop, scene_state}, state, _now, rule) do
    auto_select = Map.get(scene_state, :auto_select, false)

    if auto_select do
      case scene_state do
        %{choices: [first | _]} ->
          weapon_id = current_world().entity_registry().weapons[first] ||
                        raise "Unknown weapon: #{inspect(first)}"
          GameEngine.NifBridge.add_weapon(state.world_ref, weapon_id)
          Logger.info("[LEVEL UP] Auto-selected: #{inspect(first)} -> resuming")
          update_playing_scene_state(&rule.apply_weapon_selected(&1, first))
        _ ->
          GameEngine.NifBridge.skip_level_up(state.world_ref)
  ...
  defp process_transition({:transition, {:push, mod, init_arg}, _}, state, _now, rule) do
  ...
  defp process_transition({:transition, {:replace, mod, init_arg}, _}, state, _now, rule) do
  ...
  defp maybe_log_and_cache(state, _mod, _elapsed, rule) do
    if state.room_id == :main and rem(state.frame_count, 60) == 0 do
      ...
      weapon_info =
        get_playing_scene_weapon_levels()
  ...
  # current_rule/0 は RuleBehaviour を実装したモジュールを返す。
  # 新設定キー :current_rule が優先され、旧設定キー :current_game にフォールバックする。
  ...
  # current_world/0 は WorldBehaviour を実装したモジュールを返す。
  # 新設定キー :current_world が優先され、旧設定キー :current_game にフォールバックする。
  defp current_world do
    Application.get_env(:game_server, :current_world) ||
      Application.get_env(:game_server, :current_game, GameContent.VampireSurvivor)
  end

  # Playing シーンの state を SceneManager 経由で更新する（スタック内のどの位置にあっても更新）
  defp update_playing_scene_state(fun) when is_function(fun, 1) do
    GameEngine.SceneManager.update_by_module(current_rule().playing_scene(), fun)
  end

  # Playing シーンの weapon_levels を取得する（ログ用）
  defp get_playing_scene_weapon_levels do
    Map.get(get_playing_scene_state(), :weapon_levels, %{})
  end

  # Playing シーンの state を取得する
  defp get_playing_scene_state do
    GameEngine.SceneManager.get_scene_state(current_rule().playing_scene())
  end
