defmodule Content.RollingBall.Scenes.Playing do
  @moduledoc """
    ...
    cond do
      state.cleared ->
        ...
        if next_stage > StageData.max_stage() do
          {:transition, {:replace, Content.RollingBall.Scenes.Ending, %{}}, state}
        else
          {:transition,
           {:replace, Content.RollingBall.Scenes.StageClear,
            %{next_stage: next_stage, retries_left: state.retries_left}}, state}
        end

      state.falling and ball_y(state) <= @fall_depth ->
        {:transition,
         {:replace, Content.RollingBall.Scenes.GameOver,
