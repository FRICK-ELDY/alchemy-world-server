...
**現行構成**: [native/app/Cargo.toml](../../../native/app/Cargo.toml) は `[[bin]] name = "app"` のみ。`client_desktop` は旧クレート名で、[native-restructure-migration-plan.md](../1_backlog/native-restructure-migration-plan.md) により `app/` に統合済み。
...
以下の項目は将来対応として [bin-deprecation-mix-tasks-future.md](../0_reference/bin-deprecation-mix-tasks-future.md) に記載。
