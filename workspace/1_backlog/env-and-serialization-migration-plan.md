...
> **実施済み**: §1 環境変数リネーム、§2 MessagePack → Erlang term 直列化（フェーズ A/B/C）→ [env-and-serialization-migration-completed.md](../7_done/env-and-serialization-migration-completed.md)
...
- [platform-info-crate-and-local-user-execution-plan.md](../7_done/platform-info-crate-and-local-user-execution-plan.md) — `platform_info` → `client_info` に読み替え。`client` 配下に配置する想定で更新すること
...
- [erlang-term-schema.md](../../docs/architecture/erlang-term-schema.md) — 実施済みスキーマ
- [messagepack-schema.md](../../docs/architecture/messagepack-schema.md)（非推奨・erlang-term-schema へ誘導）
- [bottleneck-prevention.md](../../policy/bottleneck-prevention.md)（ボトルネック対策方針）
- [desktop/input.md](../../docs/architecture/rust/desktop/input.md)（set_cursor_grabbed の現状・ESC と contents 層の責務）
