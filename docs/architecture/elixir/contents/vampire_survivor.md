...
def components do
  [
    ...
    Contents.Components.Category.Rendering.Render # DrawCommand・Camera・UiCanvas → FrameBroadcaster.put
  ]
end
```
...
ワールド初期化・エンティティ登録を担当する共有コンポーネント。
...
描画データの組み立てと Zenoh 配信を担当。`on_nif_sync/1` で `content.build_frame/2` を呼び出し、DrawCommand リスト・CameraParams・UiCanvas を組み立て、`Content.MessagePackEncoder.encode_frame/4` で MessagePack 化し `FrameBroadcaster.put` で Zenoh へ publish する。FrameBuilder・SpriteParams を参照してスプライト配置や HUD を決定する。

---

## EntityParams（Elixir 側パラメータテーブル）

`Content.VampireSurvivor.EntityParams` がワールド・エンティティパラメータを保持。`Content.EntityParams` が敵 EXP・ボス EXP・スコア等の共通パラメータを提供。
