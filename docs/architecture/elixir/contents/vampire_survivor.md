...
def components do
  [
    ...
    Content.VampireSurvivor.RenderComponent   # DrawCommand・Camera・UiCanvas の push_render_frame
  ]
end
```
...
描画データの組み立てと `push_render_frame` NIF 呼び出しを担当。`on_nif_sync/1` で DrawCommand リスト・CameraParams・UiCanvas を組み立て、RenderFrameBuffer に書き込む。SpriteParams・WeaponFormulas を参照してスプライト配置や HUD を決定する。
