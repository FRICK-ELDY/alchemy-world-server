...
最終ゴールは [vision-goal.md](../vision-goal.md) の **分散連合型 VRSNS**。現行コードは主に **単一運営者内のスケールアウト層**（libcluster / `Network.Distributed` / Zenoh）まで。**運営者間の連合層**は Phase 4 で本格実装予定で、現状は未実装。詳細は [federation.md](./federation.md)。

```mermaid
graph TB
    subgraph Federation["連合層（Phase 4・未実装）"]
        S2S[S2S API / ポリシーフィルタ]
        IDF[identity federation / JWKS]
    end

    subgraph ScaleOut["スケールアウト層（現状）"]
        subgraph Server["サーバー（Elixir 主）"]
            GN[network Zenoh / UDP / Phoenix]
            ND[Network.Distributed]
        end
        GN --> ND
    end

    Federation -.->|将来| ScaleOut
```
...
├── 3rdparty/alchemy-protocol/ # Git submodule: 上流の proto/（ワイヤ契約の SSoT）
└── assets/
```
...
- [連合層アーキテクチャ](./federation.md)（二層分離・Phase 4 正本）
