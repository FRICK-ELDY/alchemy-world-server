...
`physics` は **かつて nif クレート内にあったモジュール**（移行前リポジトリでは `native/nif/src/physics/`）です。60Hz 固定の物理演算・空間ハッシュ・ECS・外部注入パラメータテーブルを担当していた。`nif` が Elixir からの NIF 経由で状態を注入・読み取っていた。

- **履歴上のパス（現行ツリーに無い）**: `native/nif/src/physics/`（`rust/` 移行後のコミットより前のツリー）
