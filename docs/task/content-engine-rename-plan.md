...
### フェーズ 2: `game_nif` のリネーム

| # | タスク | 対象 |
|:---:|:---|:---|
| 2.1 | `game_nif` → `nif` | ディレクトリ、Cargo.toml、`native/Cargo.toml` members |
| 2.2 | `nif` の内部参照 | `game_physics` → `physics`、`game_audio` → `audio` 等（Phase 1 で変更済みのクレート名） |
| 2.3 | `game_input_openxr` → `input_openxr` | `nif` の features 内の参照（Phase 1.5 でクレート名変更済み） |

---

### フェーズ 3: Elixir アプリのリネーム（core から）

`core`（旧 game_engine）が NIF をロードするため、core を先にリネームする。

| # | タスク | 対象 |
|:---:|:---|:---|
| 3.1 | `game_engine` → `core` | ディレクトリ `apps/game_engine` → `apps/core` |
| 3.2 | モジュール `GameEngine.*` → `Core.*` | 全ファイル。`lib/game_engine` → `lib/core` |
| 3.3 | mix.exs | `app: :core`、Rustler の `otp_app`、`crate`、`path` |
| 3.4 | config | `config :game_engine, ...` → `config :core, ...` |
| 3.5 | 他アプリの依存 | `{:game_engine, in_umbrella: true}` → `{:core, in_umbrella: true}` |
| 3.6 | 環境変数・コメント | `GAME_NETWORK_PORT` 等は Phase 4 で検討。NIF 関連は `core` に統一 |

---

### フェーズ 4: Elixir アプリのリネーム（network / server）

| # | タスク | 対象 |
|:---:|:---|:---|
| 4.1 | `game_network` → `network` | ディレクトリ、モジュール、mix.exs、config |
| 4.2 | `game_server` → `server` | 同上。`config :game_server, :current` → `config :server, :current` 等 |
| 4.3 | 環境変数 | `GAME_NETWORK_PORT` → `NETWORK_PORT`、`GAME_NETWORK_UDP_PORT` → `NETWORK_UDP_PORT` 等 |
