...
### フェーズ 5: Elixir アプリのリネーム（contents）✅ 完了

| # | タスク | 対象 | 状態 |
|:---:|:---|:---|:---:|
| 5.1 | `game_content` → `contents` | ディレクトリ、`lib/game_content` → `lib/contents`、`test/game_content` → `test/content` | ✅ |
| 5.2 | モジュール `GameContent.*` → `Content.*` | 全ファイル | ✅ |
| 5.3 | mix.exs | `app: :contents`、依存は既に `{:core, ...}` | ✅ |
| 5.4 | config | `GameContent.SimpleBox3D` → `Content.SimpleBox3D` 等 | ✅ |
| 5.5 | 他アプリの依存 | `{:game_content, ...}` → `{:contents, ...}` (server) | ✅ |
