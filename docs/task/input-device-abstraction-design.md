...
> 作成日: 2026-03-03  
> 目的: デバイス入力を render から分離し、Elixir を SSoT として意味論的イベントを扱う設計を定義する。VR・トラッカー等の特殊デバイスも同一の抽象で扱えるようにする。
...
- `render`（`window.rs`）がキーマッピング（WASD→move_input、Shift→sprint 等）を担っている
- 実装ルール上、`render` の責務は「描画パイプライン・HUD」であり、ゲームロジックを持たない
...
┌─────────────────────────────────────────────────────────────┐
│ Rust                                                        │
│  input ← イベントループ・ウィンドウ・生イベント取得          │
│  nif        ← RenderBridge 実装・Elixir 送信                │
│  render     ← 描画パイプライン・HUD（描画専用）              │
...
1. イベントループの所有権を `render` から `nif` または新規 `input` へ移行
2. `render` は描画専用に
...
| Phase 2: イベントループの分離 | ✅ 完了 | input にイベントループ移行、render は描画専用 |
| Phase 3: VR・トラッカー対応 | ✅ 基盤完了 | イベントフロー・Elixir 受信完了。OpenXR 実装は TODO |

### 実装ファイル

- `native/input/` — イベントループ・ウィンドウ・入力取得（winit）
- `native/render/src/window.rs` — RenderBridge トレイト・型定義のみ（描画専用）
- `native/nif/src/render_bridge.rs` — Elixir への raw_key 送信、input 経由で起動
- `native/nif/src/key_map.rs` — KeyCode → アトム名マッピング
- `native/nif/src/xr_bridge.rs` — XR イベントの Elixir エンコード・送信（`xr` フィーチャー時）
- `native/input_openxr/` — OpenXR 用クレート。`run_xr_input_loop` の骨格実装済み
- `apps/core/lib/core/game_events.ex` — raw / VR イベント受信・転送
- `apps/core/lib/core/input_handler.ex` — キー→意味のマッピング

### Phase 3 の有効化

VR 入力を有効にするには、nif を `--features xr` でビルドする:

```bash
cd native && cargo build -p nif --features xr
```
