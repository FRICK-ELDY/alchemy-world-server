...
┌─────────────────────────────────────────────────────────────┐
│ Rust                                                        │
│  game_input / game_nif ← 生イベント取得・抽象イベント送信    │
...
| Phase 2: イベントループの分離 | 未着手 | game_render が描画専用になるよう移行 |
| Phase 3: VR・トラッカー対応 | スタブのみ | `game_input_openxr` クレート作成済み |

### 実装ファイル

- `native/game_render/src/window.rs` — 生イベント転送（on_raw_key, on_raw_mouse_motion, on_focus_lost）
- `native/game_nif/src/render_bridge.rs` — Elixir への raw_key 送信
- `native/game_nif/src/key_map.rs` — KeyCode → アトム名マッピング
- `native/game_input_openxr/` — OpenXR 用クレート（Phase 3 用スタブ）
- `apps/game_engine/lib/game_engine/input_handler.ex` — キー→意味のマッピング
- `apps/game_engine/lib/game_engine/game_events.ex` — raw イベント受信・転送
