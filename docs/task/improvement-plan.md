...
| IP-03 | NIF の `unwrap()` / `expect()` を `NifResult<T>` に統一 | +2 | 小 | ✅ 完了 |
| IP-04 | Rust → Elixir メールボックスへのバックプレッシャー実装 | +2 | 小 | ✅ 完了 |
| IP-05 | ヘッドレス/オフスクリーンレンダリングモードの追加 | +2 | 中 | ✅ 完了 |
...
### IP-03: NIF の `unwrap()` / `expect()` を `NifResult<T>` に統一 ✅ 完了（2026-03-01）

> **注記**: `create_world()` の戻り値が `NifResult<T>` でラップされていない点が残存している。
> 現時点でパニックリスクは低いため暫定完了とするが、詳細は `pending-issues.md` 課題15 を参照。
...
### IP-04: Rust → Elixir メールボックスへのバックプレッシャー実装 ✅ 完了（2026-03-01）
...
### IP-05: ヘッドレス/オフスクリーンレンダリングモードの追加 ✅ 完了（2026-03-01）
