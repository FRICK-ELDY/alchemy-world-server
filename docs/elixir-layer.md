...
Elixir 側は **Umbrella プロジェクト** として構成され、4 つのアプリケーションに分割されています。ゲームの制御フロー・イベント配信・セーブ/ロードを担当します（シーン管理は contents 層の `Contents.SceneStack` が担当）。
...
### `Contents.SceneBehaviour` — シーンコールバック定義（contents 層）
...
### `Contents.SceneStack` — シーンスタック管理 GenServer（contents 層）
