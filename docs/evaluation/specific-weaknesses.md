...
- **AsteroidArena のテストがゼロ** `-2`
  > `contents` の7テストファイルは全て `VampireSurvivor` 向けであり、`AsteroidArena` に対するテストが一切存在しない。`SplitComponent`・`AsteroidArena.SpawnSystem` 等の動作が未検証。
...
- **build_instances 関数の重複（DRY 違反）** `-3`
  ...
  > 対象ファイル: `native/render/src/renderer/mod.rs`（L719-906）, `native/render/src/headless.rs`（L556-715）
...
- **nif・render・audio の Rust テストがゼロ** `-3`
  > NIF ブリッジ・描画パイプライン・オーディオの Rust テストが一切存在しない。「GPU・音声デバイスが必要なため除外」という理由があるが、`headless.rs` が存在するにもかかわらずレンダリングテストが書かれていない。`nif` の `decode_enemy_params` 等のデコードロジックはGPU不要でテスト可能。
