...
`GameNetwork.Local` の実装は完成しているが、`GameServer.Application` が起動するのは `:main` の 1 ルームのみ。`game_network_local_test.exs` でのOTP隔離テストは `StubRoom` を使用しており、実際の `GameEvents` プロセスを複数起動した際の動作が未検証。将来の拡張を阻害する。
