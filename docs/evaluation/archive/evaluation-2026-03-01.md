...
一方で、**Elixirテストカバレッジの致命的な欠如**・**networkが実質スタブ**・**WebSocket認証未実装**という3点が、プロジェクトの価値命題を損なう重大な欠如として残っている。
...
### apps/core（エンジンコア・OTP設計・コンポーネント・シーン管理）
...
- **バックプレッシャー設計（整合性維持とスキップの明確な分離）** `+5`
  ...
  > 対象ファイル（当時）: `apps/core/lib/core/game_events.ex`（L194-291）。現行: `apps/contents/lib/events/game.ex`（`Contents.Events.Game`）

- **SSoT 整合性チェック（SSOT CHECK）** `+4`
  > 60フレームごとにRust側とElixir側の値を比較し乖離を検出。設計原則を実行時に自動検証する機構。
  > 対象ファイル（当時）: `game_events/diagnostics.ex`（現行は削除または統合）
...
- **フレームループの処理順序設計** `+3`
  ...
  > 対象ファイル（当時）: `game_events.ex`（L251-297）。現行: `apps/contents/lib/events/game.ex`
...
- **SceneManager がシングルトン（マルチルーム非対応）** `-3`
  > `Contents.Events.Game` はマルチルーム対応だが `SceneManager` はシングルトン。`:main` 以外のルームはシーン処理をスキップする実装になっており、設計上の矛盾がある。
  > 対象ファイル: `apps/core/lib/core/scene_manager.ex`（L11）

- **Contents.Events.Game に BatLord 固有ロジックが漏出** `-2`
  > `handle_info({:boss_dash_end, world_ref})` がエンジンコアに実装されており、実装ルール違反。
  > 対象ファイル（当時）: `game_events.ex`（L180-187）。現行: `apps/contents/lib/events/game.ex`
...
- **GameEngine コアのテストがゼロ** `-4`
  > `SceneManager`・`Contents.Events.Game`・`EventBus`・`SaveManager` に対するテストが一切存在しない。

**小計: +24 / -11 = +13点**

---

### apps/contents（コンテンツ実装・ゲームロジック・パラメータ設計）

#### ✅ プラス点

- **純粋関数による World/Rule 実装** `+4`
  > `BossSystem`・`SpawnSystem`・`LevelSystem` が全て純粋関数として実装されており、副作用がない。
  > 対象ファイル: `apps/contents/lib/contents/vampire_survivor/`
...
- **EntityParams と SpawnComponent のパラメータ二重管理** `-3`
  ...
  > 対象ファイル: `apps/contents/lib/contents/entity_params.ex`, `spawn_component.ex`
...
- **WebSocket 認証・認可が未実装** `-3`
  > 誰でも任意のルームに参加できる状態。
  > 対象ファイル: `apps/network/lib/network/channel.ex`
...
### native/physics（ECS・SoA・SIMD・衝突・決定論性）
...
- **bench/chase_ai_bench.rs のクレート名不一致（コンパイル不可）** `-3`
  > `game_simulation` クレートをインポートしているが実際のパッケージ名は `physics`。ベンチマークがコンパイルできない状態。
...
### native/desktop_render（描画パイプライン・シェーダー・補間）
...
- **wgpu インスタンス描画（1 draw_indexed で全スプライト）** `+4`
  ...
  > 対象ファイル: `native/desktop_render/src/renderer/mod.rs`（L51-58, L991）
...
- **RenderBridge トレイトによる疎結合** `+3`
  ...
  > 対象ファイル: `native/desktop_render/src/window.rs`（L29-33）

#### ❌ マイナス点

- **build_instances 関数の重複（DRY 違反）** `-3`
  > `renderer/mod.rs` と `headless.rs` にほぼ同一のスプライトUV・サイズ計算ロジックが重複。
  > 対象ファイル: `native/desktop_render/src/renderer/mod.rs`（L719-906）, `headless.rs`（L556-715）

- **Skeleton/Ghost の UV がプレースホルダー** `-2`
  > 別エンティティとして存在するにもかかわらず視覚的に区別できない。
  > 対象ファイル: `native/desktop_render/src/renderer/mod.rs`（L258-266）
...
### native/audio（非同期設計・フォールバック・アセット管理）
...
### native/nif（NIF設計・Elixir×Rust ブリッジ・SSoT一貫性）
...
- **純粋関数テストの徹底** `+3`
  > `contents` の7テストファイルが全て純粋関数・ロジック部分のみをテスト。
...
- **nif・render・audio の Rust テストがゼロ** `-3`
  > NIF ブリッジ・描画パイプライン・オーディオの Rust テストが一切存在しない。`nif` のデコードロジックはGPU不要でテスト可能。
...
- **lock_metrics による RwLock 待機時間の可観測性（再掲）** — 既にnifで計上
...
- **オーディオ・NIF・ネットワーク各層のフォールバック戦略** `+3`
  > `audio` のデバイス不在フォールバック・`nif` の `lock_poisoned_err()` / `params_not_loaded_err()` フェイルファスト・`network` の UDP 起動失敗時の `Logger.error` が揃っている。
...
- **improvement-plan.md による課題の一元管理** `+3`
...
- **improvement-plan.md による自己評価サイクル** `+3`
  ...
  > 対象ファイル: `docs/plan/reference/improvement-plan.md`
...
|:---|:---:|:---:|:---:|
| apps/core | +24 | -11 | **+13** |
| apps/contents | +11 | -8 | **+3** |
| apps/network | +8 | -7 | **+1** |
| apps/game_server | +3 | 0 | **+3** |
| native/physics | +26 | -5 | **+21** |
...
2. **network が実質スタブ** — プロジェクトの価値命題（「なぜElixir + Rustか」）がコードで証明されていない
