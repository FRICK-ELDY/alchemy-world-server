...
| **プラス** | MessagePack デコードが網羅的（DrawCommand, CameraParams, UiCanvas, MeshDef 等）。put_drop で CongestionControl::Drop。ClientInfo の発行 |
...
5. **network** — MessagePack デコードと NetworkRenderBridge の完成度
...
**native の強み**は nif/physics の SoA・SIMD 設計と、network の MessagePack/NetworkRenderBridge、audio のコマンド駆動である。**弱点**は shared の未完成、xr の未実装、network→render→nif のアーキテクチャ違反である。
