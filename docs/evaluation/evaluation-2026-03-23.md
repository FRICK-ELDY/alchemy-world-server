...
- **テスト存在確認**: `EventBus`・`SaveManager`（現行は削除済み）のテストが `apps/core/test/` に存在することを確認（当時の記録）
- **Contents.Scenes.Stack / Contents.Events.Game のテスト**: `apps/core/test/` 内に当該モジュールのテストは**依然として存在しない**ことを確認
...
| Contents.Scenes.Stack・Contents.Events.Game テスト | 未整備のまま |
...
引き続き、[improvement-plan.md](../task/improvement-plan.md) に従い、**pull_request トリガー追加**・**Contents.Scenes.Stack・Contents.Events.Game テスト整備**・**EntityParams SSoT 化**を優先して進めることを推奨する。
