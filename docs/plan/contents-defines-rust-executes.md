...
| **wgpu ランタイムコンパイル** | ✓ 技術的に可能 | `create_shader_module()` で WGSL を実行時コンパイル可能。現状は `include_str!` を使用 |
...
| 課題 | 内容 |
|:---|:---|
| Elixir から WGSL を渡すインターフェース | P4-2: アセットパス or 文字列注入の設計・実装 |
| コンテンツ内で WGSL をアセットとして保持 | P4-3: `content.assets_path()` 配下の構成検討 |
| NIF 経由でシェーダーを取得するパス | P4-4: 起動時ロード or 動的登録の実装 |
| `include_str!` の扱い | P4-5: フォールバック残存か完全移行かの決定 |
