...
| Phase 3 | 完全フレックス（任意 layout） | 検討のみ。実際の実装範囲は Phase 2 までとする |
...
| 項目 | 状態 | 説明 |
|:---|:---|:---|
| **式（Formula）** | ✓ 実装済み | Elixir が FormulaGraph を定義、Rust VM が実行。パラメータ計算の SSoT は Elixir |
| **DrawCommand** | ✓ 実装済み | Elixir が組み立て、NIF で Rust に渡し、decode して描画。定義は [draw-command-spec.md](../architecture/draw-command-spec.md) |
| **シェーダー契約の文書化** | ✓ 完了（P4-1） | sprite.wgsl / mesh.wgsl の uniform・バインド・頂点レイアウトを [render.md](../architecture/rust/render.md) に SSoT 化 |
| **シェーダー Elixir 移行** | ✓ 完了（P4-2〜5） | `assets/{game_id}/shaders/*.wgsl` から起動時ロード。未存在時は `include_str!` フォールバック。[shader-elixir-interface.md](../architecture/shader-elixir-interface.md) |
| **メッシュ定義の Elixir 移行** | 一部 | P3 で mesh_def を渡して unit_box / skybox_quad を使用可能。未登録時は Rust 側フォールバック |
...
| 課題 | 内容 |
|:---|:---|
| Elixir から WGSL を渡すインターフェース | P4-2 完了: アセットパス方式で起動時ロード |
| コンテンツ内で WGSL をアセットとして保持 | P4-3 完了: `assets/{game_id}/shaders/*.wgsl` |
| NIF 経由でシェーダーを取得するパス | P4-4 完了: atlas_path から導出し render_bridge でロード |
| `include_str!` の扱い | P4-5 完了: フォールバックとして残す |
...
| 全メッシュの Elixir 定義移行 | box_mesh / grid_lines / skybox_verts のフォールバック廃止 |

### 6.3 その他

| 課題 | 内容 |
|:---|:---|
| Formula の徹底 | P1: ハードコード洗い出し・武器式等の Formula 移行 |
| 転送効率化 | P5: バッチ API、バイナリ形式、decode オーバーヘッド低減 |
| セキュリティ | シェーダー注入時のサンドボックス化（将来の不信頼コンテンツ利用時） |
