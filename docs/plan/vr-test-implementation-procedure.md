...
> `native/input_openxr` を使って VR ヘッドセットで3D空間を見渡せるようにする。
...
| コンポーネント | 状態 | 備考 |
|:---|:---|:---|
| `native/input_openxr` | ✅ 実装済み | XR_MND_headless でヘッドレスセッション、head pose 取得 |
| `native/nif` xr_bridge | 完了 | XrInputEvent → Elixir タプル変換・送信は実装済み |
| `apps/core/game_events.ex` | 完了 | `{:head_pose}`, `{:controller_pose}` 等の `handle_info` は実装済み |
| `native/render` | デスクトップのみ | winit + wgpu でウィンドウ描画。VR ステレオレンダリング・OpenXR コンポジションは未対応 |
| `config :core, Core.NifBridge, features` | デフォルト `[]` | VR 有効化には `features: ["xr"]` が必要 |

### 1.2 データフロー（設計済み）

```
OpenXR runtime → input_openxr → xr_bridge → GameEvents
                                                      ↓
...
1. `native/input_openxr` に OpenXR クレートの依存を確認（`openxr` 0.21）
...
1. `config/config.exs` で `config :core, Core.NifBridge, features: ["xr"]` を設定
2. `apps/core/lib/core/game_events.ex` の `init/1` 内で、`render_started` が true のときに `Core.NifBridge.spawn_xr_input_thread(self())` を呼ぶ
...
- [ ] VR ヘッドセットが接続されている
- [ ] `config :core, Core.NifBridge, features: ["xr"]` を設定済み
- [ ] `mix compile` が `--features xr` で成功する
- [ ] `native/input_openxr` の `openxr` フィーチャーが有効になっている（nif の xr フィーチャー経由）
