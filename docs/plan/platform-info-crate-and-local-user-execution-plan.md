...
> 作成日: 2026-03-09  
> 目的: クライアント情報（OS, arch 等）を `native/client` の `client_info` モジュールで取得し、Zenoh 経由（Erlang term 形式）で Elixir と通信、LocalUserComponent で取得、MenuComponent で全ワールドのメニューに表示する。
...
| 項目         | 内容                                                                          |
| ---------- | --------------------------------------------------------------------------- |
| **配置**      | `native/client` クレート内の `client_info` モジュール（[env-and-serialization-migration-plan](./env-and-serialization-migration-plan.md) §5 参照） |
...
## 3. フェーズ 1: native/client 内に client_info モジュール作成

※ `native/client` の作成は [env-and-serialization-migration-plan](./env-and-serialization-migration-plan.md) §5 に従う。

### 3.1 client_info モジュール作成

`native/client/src/client_info.rs` を追加:
...
fn publish_client_info(&self, room_id: &str) {
    let info = client::client_info::ClientInfo::current();
...
- 専用 NIF 関数 `get_client_info/0` で `client::client_info::ClientInfo::current()` を返す（Erlang term 形式でエンコード）
...
| 順序  | フェーズ               | 内容                                                                              |
| --- | ------------------ | ------------------------------------------------------------------------------- |
| 1   | native/client 作成     | `client` クレート作成、`client_info` モジュール追加（[env-and-serialization-migration-plan](./env-and-serialization-migration-plan.md) §5 参照） |
...
| 6   | （任意）NIF モード        | NIF から `client::client_info::ClientInfo` を取得し、`:client_info` に保存するパスを追加        |
