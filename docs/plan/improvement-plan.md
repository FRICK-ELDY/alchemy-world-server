...
現状の実装ではランチャー単体では動作しない。インストール構成・zenohd 同梱・HL-Server の Elixir release 化・desktop_client のパス指定などが必要。`config/launcher.toml` でパス上書き可能にすればリリース構成に対応可能。
...
- [client-server-separation-procedure.md](./client-server-separation-procedure.md) — クライアント・サーバー分離（zenohd トレイ統合）
