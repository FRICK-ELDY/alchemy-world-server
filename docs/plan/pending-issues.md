...
- `GamePhase` 列挙型に `StageClear` / `Ending` を追加（`native/desktop_render/src/renderer/mod.rs`）
...
- `native/desktop_render/src/renderer/mod.rs` — `GamePhase` の汎用化
- `native/desktop_render/src/renderer/ui.rs` — 汎用オーバーレイ UI の実装
...
### 課題19: GameWorldInner → ContentsInner と計算式・アルゴリズムの Rust 実行

**優先度**: 低（将来方針）

**背景・方針**

PlayerState 移行や Boss SSoT 移行と同様、状態の所有権を contents 層に寄せる方向性を進める。さらに発展として：

1. **GameWorldInner → ContentsInner**
   Rust 側の「ゲームワールド内部状態」を、contents が定義する「コンテンツ内部状態」として扱う方向へ移行する。

2. **Contents が計算式・アルゴリズムを持ち、Rust で実行する**
   コンテンツ層が計算式やアルゴリズムを**定義**し、Rust がそれを**実行**する役割分担にしたい。Elixir は「何を計算するか」、Rust は「どう計算するか」を持つ。

**目標イメージ**

例として、以下のような命令列を Elixir から Rust に送り、結果を受け取る形を想定する：
...
つまり、コンテンツは命令列（バイトコード的な表現）で計算ロジックを記述し、Rust 側に命令実行エンジンを持つ。これにより：

- コンテンツは計算内容の SSoT
- Rust は演算の高速実行に特化
- NIF 境界では「命令列 + 入力」「結果」のみ受け渡し
