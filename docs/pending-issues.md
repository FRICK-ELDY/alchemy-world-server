...
- `apps/core/lib/core/game_events.ex` — NIF エラー回復ロジック
...
- `apps/core/lib/core/game_events.ex` — `Network` へのブロードキャスト追加
...
`Core.SceneManager.get_scene_state/1` を呼ぶ際に
...
- `apps/core/lib/core/scene_manager.ex`（案C の場合）
...
- `apps/core/lib/core/game_events.ex` — `move_input` / UI アクションの配信整理
...
`Core.GameEvents.Diagnostics` の `do_log_and_cache/3` が
...
- `apps/core/lib/core/game_events/diagnostics.ex` — 暫定コードの除去・コールバック呼び出しへの置き換え
