...
`mix run` 単体ではウィンドウは開かず、サーバーのみ起動します。ゲームをプレイするには zenohd と client_desktop を別途起動してください。
...
- **Client Run** → zenohd と Phoenix Server の起動を確認してから client_desktop を起動
...
3. ターミナル 3: デスクトップクライアントを起動
   ...
   # Linux / macOS
   cargo run -p app --bin client_desktop -- --connect tcp/127.0.0.1:7447 --room main
   ```
